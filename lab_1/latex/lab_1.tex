...
\begin{document}
    ...
    % Выполняя задание, я пользовался данными с википедии на момент 20.10.2024, где те же параметры обозначаются другими буквами.
    % Поэтому выясним сразу, как соотносятся обозначения на википедии с нашими. Распределение Лапласа с википедии имеет вид
    % $$f(x)=\dfrac{\alpha}{2}\exp{\left\{-\alpha|x-\beta|\right\}},$$ где $-\infty<\beta<+\infty$ -- параметр сдвига, следовательно
    % $$\beta=\mu,$$ а $\alpha>0$ -- параметр масштаба. Соотнеся функцию распределения Лапласа из википедии с данной в указании в условии
    % задания получим и решим следующую систему
    % $$
    % \begin{cases}
    %     \frc{\alpha}{2}=\frc{1}{2\theta}\\
    %     \alpha=\frc{1}{\theta}
    % \end{cases}\Rightarrow
    % \,\,\,\,\,\alpha=\dfrac{1}{\theta}
    % $$
    % Теперь мы знаем, что $\beta=\mu$ и $\alpha=\frc{1}{\theta}$.
    ...
    Вычислим теоретические дисперсию (Var) и среднеквадратическую ошибку (MSE). Так как оценка является несмещенной,
    то $$\text{Var}\,{\left[\hat{\theta}^2\right]}=\text{MSE}\,{\left[\hat{\theta}^2\right]},$$ а значит нам достаточно
    найти $\text{Var}\,{\left[\hat{\theta}^2\right]}$. Покажем это -- для вычисления среднеквадратической ошибки используется следующая
    формула $$\text{MSE}\,{\left[\hat{\theta}\right]}=\mathbb{E}\left[\left(\hat{\theta}-\theta\right)^2\right]
    =\text{Var}\,{\left[\hat{\theta}\right]}+\text{bias}\,{\left[\hat{\theta},\,\theta\right]}^2$$
    В нашем случае формула будет иметь вид
    $$\text{MSE}\,{\left[\hat{\theta}^2\right]}=\text{Var}\,{\left[\hat{\theta}^2\right]}+\text{bias}\,{\left[\hat{\theta}^2,\,\theta^2\right]}^2,$$
    так как смещение равно нулю, то второе слагаемое обнуляется, получаем
    $$\text{MSE}\,{\left[\hat{\theta}^2\right]}=\text{Var}\,{\left[\hat{\theta}^2\right]}+0^2=\text{Var}\,{\left[\hat{\theta}^2\right]}$$
    Используя свойства дисперсии вычислим ее значение
    $$\text{Var}\,{\left[\hat{\theta}^2\right]}=\text{Var}\,{\left[\dfrac{1}{2n}\sum\limits_{j=1}^{n}x_j^2\right]}
    =\dfrac{1}{4n^2}\text{Var}\,{\left[\sum\limits_{j=1}^{n}x_j^2\right]}=\dfrac{1}{4n^2}\sum\limits_{j=1}^{n}\text{Var}\,{\left[x_j^2\right]},$$
    $$\sum\limits_{j=1}^{n}\text{Var}\,{\left[x_j^2\right]}=n\cdot\text{Var}\,{\left[x^2\right]}\Rightarrow \dfrac{1}{4n^2}\sum\limits_{j=1}^{n}\text{Var}\,{\left[x_j^2\right]}
    =\dfrac{1}{4n^2}\cdot n\cdot\text{Var}\,{\left[x^2\right]}=\dfrac{\text{Var}\,{\left[x^2\right]}}{4n},$$
    $$\text{Var}\,{\left[x^2\right]}=\mathbb{E}\left[x^4\right]-\left(\mathbb{E}\left[x^2\right]\right)^2,$$
    $$\left(\mathbb{E}\left[x^2\right]\right)^2=\left(2\theta^2\right)^2=4\theta^4,\ \ \mathbb{E}\left[x^4\right]=\int\limits_{-\infty}^{\infty}x^4f(x)\,dx,$$
    $$\int\limits_{-\infty}^{\infty}x^4f(x)\,dx=\int\limits_{-\infty}^{\infty}x^4\dfrac{1}{2\theta}\exp{\left\{-\dfrac{|x|}{\theta}\right\}}\,dx=2\int\limits_{0}^{\infty}x^4\dfrac{1}{2\theta}\exp{\left\{-\dfrac{x}{\theta}\right\}}\,dx=
    ...
    $$\Gamma{(z)}=\int\limits_{0}^{\infty}t^{z-1}e^{-t}\,dt\Rightarrow\int\limits_{0}^{\infty}u^4e^{-u}\,du=\Gamma{(z-1=4)}=\Gamma{(5)}=24\Rightarrow\theta^4\cdot\Gamma{(5)}=\theta^4\cdot24,$$
    $$\text{Var}\,{\left[x^2\right]}=24\theta^4-4\theta^4=20\cdot\theta^4\Rightarrow\text{Var}\,{\left[\hat{\theta}^2\right]}=\dfrac{20\cdot\theta^4}{4n}=\dfrac{5\theta^4}{n}
    =\dfrac{5\cdot0.5^4}{n}=\dfrac{0.3125}{n},$$
    Таким образом
    $$\text{MSE}\,{\left[\hat{\theta}^2\right]}=\text{Var}\,{\left[\hat{\theta}^2\right]}=\dfrac{0.3125}{n}$$
    Видим, что при увеличении объема выборки $n$ разброс и ошибка будут уменьшаться.
    ...
    $$\lim\limits_{n\rightarrow\infty}\text{Var}\,{\left[\hat{\theta}^2\right]}=\lim\limits_{n\rightarrow\infty}\text{MSE}\,{\left[\hat{\theta}^2\right]}=\lim\limits_{n\rightarrow\infty}\dfrac{0.3125}{n}=0,$$
