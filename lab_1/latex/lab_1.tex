...
\begin{document}
    ...
    из столбца, используя методы библиотеки \texttt{pandas} 
    ...
    Зададим сразу все три выборки -- всю, только модели с наличием Wi-Fi и только с отсутствием.
    Библиотека \texttt{pandas} позволяет удобно отбирать нужные значения -- для выборок с Wi-Fi
    оставляем все строки, для которых выполняется равенство единице или нулю.
    Добавим для удобства метод разделитель, который будем вызывать между выборками.
    ...
    Результаты получились хорошие. Теперь построим графики в соответствии с заданием.
    ...
    \texttt{matplotlib} для отрисовки. Для подсчета необходимых данных все также
    нужна библиотека \texttt{pandas}. Для построения графика эмпирической функции распределения находим
    по сортированным данным без сохранения индексов ключ-значение, где ключ --
    \texttt{battery\_{power}}, значение -- вероятность встретить именно такую \texttt{battery\_{power}}.
    Однако для графика нужны не сами значения, а кумулятивные суммы. Для гистограммы определяем
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{all_battery_box.png}
        \captionsetup{skip=0pt}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{wifi_battery_box.png}
        \captionsetup{skip=0pt}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{no_wifi_battery_box.png}
        \captionsetup{skip=0pt}
