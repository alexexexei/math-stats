...
\begin{document}
    ...
    \texttt{theta} -- истинное значение масштабирующего параметра, \texttt{delta} -- порог для определения смещения, \texttt{hat\_theta2\_list}
    ...
    for n in n_list:
        for i in range(m):
            # generating a sample from a Laplace
            # distribution with zero bias
            sample = np.random.laplace(loc=0, scale=theta, size=n)
                
            # calculating hat_theta^2 
            # accroding to the derived formula
